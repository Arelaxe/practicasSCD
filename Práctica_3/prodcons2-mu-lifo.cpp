...
// varios productores y consumidores. Opción fifo
//
// -----------------------------------------------------------------------------
...
const int
   ...
   num_items             = 20,
   tam_vector            = 10,
   valores_por_productor = num_items / num_prods,
   etiq_prod             = 0,
   etiq_cons             = 1;
...
{
   static int contador = orden_prod * valores_por_productor;
...
{
   for ( unsigned int i= 0 ; i < valores_por_productor ; i++ )
   {
      // producir valor
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      switch( estado.MPI_TAG ) // leer emisor del mensaje en metadatos
      {
         ...
         case etiq_cons: // si ha sido el consumidor: extraer y enviarle
            valor = buffer[num_celdas_ocupadas-1] ;
            num_celdas_ocupadas-- ;
...
{
   int id_propio, num_procesos_actual, orden_prod, orden_cons;
