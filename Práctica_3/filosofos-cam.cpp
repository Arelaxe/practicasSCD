// Noelia Escalera Mejías 09079064K
...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
		cout << "Filósofo " << id << " pide permiso al camarero para sentarse." << endl;
    ...
    MPI_Ssend (&peticion ,1, MPI_INT, id_ten_der, 0, MPI_COMM_WORLD);
	  ...
	  cout << "Filósofo " << id << " pide permiso para levantarse " << endl;
...
void funcion_camarero(){
	int peticion, etiq_valida;
	int filos_sentados = 0;
	MPI_Status estado;
	...
	while ( true ){
		...
		MPI_Recv(&peticion, 1, MPI_INT, MPI_ANY_SOURCE, etiq_valida, MPI_COMM_WORLD, &estado);

		if (estado.MPI_TAG == etiq_sentarse){
			cout << "El camarero da permiso al filósofo " << estado.MPI_SOURCE << " para sentarse." << endl;
		  filos_sentados++;
		}
		else if (estado.MPI_TAG == etiq_levantarse){
			cout << "El camarero da permiso al filósofo " << estado.MPI_SOURCE << " para levantarse." << endl;
			filos_sentados--;
		}
		cout << "Hay " << filos_sentados << " filósofos sentados" << endl; 
	}
}
...
int main( int argc, char** argv )
{
   ...
   if ( num_procesos == num_procesos_actual )
   {
	  ...
	  if ( id_propio == id_camarero)
		 funcion_camarero();
   	  else if ( id_propio % 2 == 0 && id_propio != id_camarero)          // si es par
